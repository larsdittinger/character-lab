...
- New references use `tools/generate_reference.py --character <id>`: OpenAI `gpt-image-2.5-sunburst`, `quality=high`, `size=1536x1024`, one image with three views. `high` is image quality, not `reasoning_effort`. The mandatory base-character rules also apply to custom prompts. Use only `OPENAI_API_KEY` in the environment or this directory's ignored `.env`; no runtime secret search in other projects, no key in client code or logs. The user-authorized one-time key setup is already complete.
