...
For a fresh clone or a different model provider, also read [docs/AGENT_START.md](docs/AGENT_START.md). A local model, ChatGPT/OpenAI or Gemini may guide the work through the user's chosen agent host. Reference images may be imported from a local image model or an image-capable Gemini/ChatGPT workflow with original bytes and provenance metadata. Only the OpenAI image API helper is built in; do not invent a Gemini/local provider flag. A text-only model needs a vision-capable collaborator or human for visual measurements and acceptance.

## New characters: required folder and accessory rules

- Generate the base character with fitted clothing and compact armor, **without a cape/cloak, loose hanging cloth, long coat tails/tabard panels, backpack, quiver, weapons, dangling straps or other protruding equipment**. Natural anatomy (including ears) stays. Model optional cloth and equipment separately under `characters/<id>/accessories/`, then attach to the accepted rig. The supplied historical characters/references are not regenerated to apply this new rule.
- Create a workspace with `.venv/bin/python tools/new_character.py <id> --title "Name"`. Keep its original references, metadata, independently measured config, geometry scripts, atlas, GLB/Blender exports, clips and review files inside `characters/<id>/`. Read [docs/CHARACTERS.md](docs/CHARACTERS.md) before creating one.
- The built-in reference generator is `tools/generate_reference.py --character <id>`: OpenAI `gpt-image-2.5-sunburst`, `quality=high`, `size=1536x1024`, one image with three views. Other reference sources follow [docs/AGENT_START.md](docs/AGENT_START.md). `high` is image quality, not `reasoning_effort`. The mandatory base-character rules apply to every provider and custom prompt. Use only `OPENAI_API_KEY` in the environment or this directory's ignored `.env`; no runtime secret search in other projects, no key in client code or logs. A fresh clone contains no credentials; each user supplies their own key only if generation is needed.
- The viewer discovers `characters/*/character.json` with a real `assets/animated.glb` or `assets/static.glb` automatically through `npm run serve`. Do not add hardcoded characters to `src/viewer.js`. A reference-only scaffold must not appear as a finished model or fictional animation library.
- Use `tools/pipeline.py <stage> --character <id>`. Cache hits require matching input and output content hashes; final numeric validation always runs. `--force` is available for full reproduction. Do not reduce geometry, texture resolution, motion samples or validation coverage to improve build times. See [docs/PERFORMANCE.md](docs/PERFORMANCE.md).

## Preserve the proven quality

Follow the **Proven iteration cycle for the next agent** at the start of [docs/WORKFLOW.md](docs/WORKFLOW.md). It records the successful iteration order and a symptom-to-fix table for all three characters. Before experimenting, preserve the current exported model, relevant code/configuration, atlas and camera-matched renders under a new baseline directory. Keep a change only after numeric checks pass and the visible improvement survives the other required views and representative motions. If it does not improve the result, restore that attempt's code/configuration and rebuild its outputs together. Do not overwrite historical comparison assets or present a new camera/lighting setup as evidence of a projection fix.
...
The optional OpenAI image helper is separate from offline builds. Use only `OPENAI_API_KEY` or this directory's `.env`; never print or commit a key. No new image is needed to reproduce the supplied assets. For a requested new reference, use the folder-based command in [docs/CHARACTERS.md](docs/CHARACTERS.md). Preserve original output bytes, full prompt, SHA-256 and API usage. One three-view sheet is one output image, not three. Verify current prices before quoting them; never claim an actual bill from a price estimate. Historical knight references remain Gemini outputs; ranger/elf references remain built-in ImageGen outputs.

Do not claim perfect production rigging or photogrammetric reconstruction. This is a stylized, individually calibrated reconstruction with projection texture, rigid glove shapes and simple cloth bones. Props, individual fingers, facial expressions, cloth collision and scene-specific foot contacts need additional work for production use.
