...
def loft(name,cfg,mirror=False):
 ...
 if cfg.get('side',False):
  if face:
   ...
   radius=np.sqrt(((px-785)/23)**2+((sy-78)/32)**2)
   mask=(1-smooth((radius-.82)/.30))[...,None]
   hair_weight=np.maximum(smooth((px-790)/17),1-smooth((sy-61)/17))[...,None]
   fill=sample(739+(px-785)*.10,np.clip(sy,65,143))*(1-hair_weight)+sample(810+(px-785)*.1,np.clip(sy,36,146))*hair_weight
 ...
 if zone=='hair':
  color=sample(np.clip(px,793,838),np.clip(yp,84,190))
 elif zone=='ear':
  # Front and profile each own their corresponding surface of the real ear mesh.
  front_ear=sample(xp,yp);profile_ear=sample(np.clip(px,755,811),np.clip(yp,50,104))
