"""Independent elf build, fitted rig, and camera-matched visual inspection.
Usage: blender -b -t 4 --python tools/elf_blender.py -- build|rig|render|actions
"""
...
def export(objects,file,yup=False):
 ...
 bpy.context.object.name='NightElf'
...
if mode=='build':
 clear();spec=json.loads((R/'assets/elf-model.json').read_text());mat=bpy.data.materials.new('Night elf / measured image projection');mat.use_nodes=True;nt=mat.node_tree;bs=nt.nodes.get('Principled BSDF');bs.inputs['Roughness'].default_value=.86;bs.inputs['Specular IOR Level'].default_value=.17
 im=nt.nodes.new('ShaderNodeTexImage');im.image=bpy.data.images.load(str(R/'textures/elf-projection.png'));im.image.pack();nt.links.new(im.outputs['Color'],bs.inputs['Base Color']);objects=[];topology=[]
 ...
 bpy.ops.wm.save_as_mainfile(filepath=str(R/'assets/elf-static.blend'));export(objects,'elf-static.glb')
 model=bpy.context.object;report=dict(triangles=sum(len(p.vertices)-2 for p in model.data.polygons),vertices=len(model.data.vertices),parts=len(objects),source='references/elf/turnaround.png')
 (R/'review/elf/build.json').write_text(json.dumps(report,indent=2));(R/'review/elf/topology.json').write_text(json.dumps(topology,indent=2));print('ELF BUILD',report)
elif mode=='rig':
 bpy.ops.wm.open_mainfile(filepath=str(R/'assets/elf-static.blend'));cfg=json.loads((R/'config/elf-rig.json').read_text());spec=cfg['bones'];source=json.loads((R/'config/source-rig.json').read_text())
 ...
 arm=bpy.data.armatures.new('Elf measured humanoid');rig=bpy.data.objects.new('ElfRig',arm);bpy.context.collection.objects.link(rig);bpy.context.view_layer.objects.active=rig;rig.select_set(True);bpy.ops.object.mode_set(mode='EDIT')
 ...
 def weights(name,p):
  ...
  if name.startswith('Cloak'):
   # Both rear cloth bones share the broad cloak; shoulders stay with the upper spine.
   left=smooth(x,-.08,.08);hang=1-smooth(y,1.34,2.03)
   return {'spine_03':1-hang,'cloth_back_l':hang*left,'cloth_back_r':hang*(1-left)}
 ...
 for o in objects:
  ...
  mod=o.modifiers.new('Fitted elf skin','ARMATURE');mod.object=rig;o.parent=rig;stats.append(dict(part=o.name,vertices=len(o.data.vertices),minWeightSum=min(sums),maxWeightSum=max(sums)))
 bpy.context.scene.render.fps=30;bpy.ops.wm.save_as_mainfile(filepath=str(R/'assets/elf-rigged.blend'));export(objects+[rig],'elf-rigged.glb',True)
 (R/'review/elf/skin.json').write_text(json.dumps(dict(bones=len(arm.bones),parts=stats),indent=2));print('ELF RIG',len(arm.bones))
elif mode=='actions':
 os.environ['CHARACTER_LAB_CHARACTER']='elf'
 runpy.run_path(str(R/'tools/save_animated.py'),run_name='__main__')
elif mode=='render':
 clear();bpy.ops.import_scene.gltf(filepath=str(R/'assets/elf-static.glb'))
 ...
 w=bpy.data.worlds.new('Elf QA');w.use_nodes=True;w.node_tree.nodes['Background'].inputs[0].default_value=(.065,.085,.09,1);w.node_tree.nodes['Background'].inputs[1].default_value=.8;scene.world=w
 ...
 for name,direction in [('front',(0,0,1)),('3q',(.8,.08,1)),('side',(1,0,0)),('back',(0,0,-1))]:
  target=Vector((0,1.25,0));d.ortho_scale=2.78;cam.location=target+Vector(direction).normalized()*5;aim(cam,target);scene.render.filepath=str(R/'review/elf'/f'body-{name}.png');bpy.ops.render.render(write_still=True)
  if name!='back':
   target=Vector((0,2.29,.035));d.ortho_scale=.61;cam.location=target+Vector(direction).normalized()*5;aim(cam,target);scene.render.filepath=str(R/'review/elf'/f'face-{name}.png');bpy.ops.render.render(write_still=True)
else:raise SystemExit('Unknown elf task: '+mode)
