...
def loft(name,cfg,mirror=False):
 ...
 if cfg.get('side',False):
  if face:
   ...
   ear=(px>771)&(px<793)&(sy>90)&(sy<124)
   ear_mix=smooth((px-771)/22)[...,None]
   fill=sample(np.full(px.shape,767.),sy)*(1-ear_mix)+sample(np.full(px.shape,797.),sy)*ear_mix
   side[ear]=fill[ear]
  elif zone=='hair':
   side=sanitize(sample(np.maximum(px,800),yp),base)
  elif name.startswith('Neck'):
   # The narrow raised collar silhouette is occluded at several source depths.
   # Continuous material bands avoid carrying antialiased backdrop into its rim.
   side=sample(np.full(px.shape,790.),yp)
