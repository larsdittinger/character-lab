...
def padded_rear_hair():
 # Extend only observed pale hair along each rear scanline; never pull cloak or gray studio pixels.
 out=src.copy();x0,x1=1175,1330
 for y in range(18,176):
  lo=np.interp(y,[18,30,50,90,130,150,160,170,175],[1247,1219,1197,1191,1200,1215,1221,1241,1248])
  hi=np.interp(y,[18,30,50,90,130,150,160,170,175],[1260,1288,1308,1314,1304,1294,1281,1272,1260])
  line=src[y,x0:x1];grid=np.arange(x0,x1)
  valid=(grid>=lo)&(grid<=hi)&(line[:,2]-line[:,0]>.016)&(line.mean(axis=1)>.46)
  xs=np.flatnonzero(valid)
  if not len(xs):continue
  idx=np.searchsorted(xs,np.arange(x1-x0));right=xs[np.minimum(idx,len(xs)-1)];left=xs[np.maximum(idx-1,0)]
  nearest=np.where(abs(np.arange(x1-x0)-left)<abs(np.arange(x1-x0)-right),left,right)
  out[y,x0:x1]=line[nearest]
 return out
rear_hair_source=padded_rear_hair()
...
def loft(name,cfg,mirror=False):
 ...
 # Broaden the skull in geometry while UV sampling stays on the measured source.
 # Ears move with the cheek but keep their authored pointed length.
 head_scale=C.get('headWidthScale',1)
 if face:pos[:,:,0]*=head_scale
 elif cfg.get('zone')=='ear':pos[:,:,0]+=(head_scale-1)*33*S
 ...
 if face:
  ...
  # Behind the ear, the profile sheet mixes ear, hair and studio background.
  # The unobstructed rear sheet owns this portion of the continuous head mesh.
  hair_edge=np.interp(yp,[18,75,120,184],[750,773,771,779])
  hair_mask=(smooth((px-hair_edge)/14)*smooth((yp-106)/18)*(1-smooth((yp-160)/16)))[...,None]
  rear_hair=sample(np.clip(rear_x,1175,1329),np.clip(yp,30,169),rear_hair_source)
  color=color*(1-hair_mask)+rear_hair*hair_mask
 if zone=='cloak':
