"""Offline copper elf rebuild from its independently measured reference."""
...
def blender(stage):
 ...
 run(exe,'-b','-t','4','--python-exit-code','1','--python',R/'tools/elf_blender.py','--',stage)
if a.task in ['build','geometry']:
 run(sys.executable,R/'tools/validate_elf_inputs.py');run(sys.executable,R/'tools/elf_model.py');blender('build')
if a.task in ['build','rig']:blender('rig')
if a.task in ['build','motion']:
 run('node',R/'tools/elf_motion.mjs','retarget');run('node',R/'tools/elf_motion.mjs','clips');blender('actions')
if a.task in ['build','validate']:run('node',R/'tools/elf_motion.mjs','validate')
