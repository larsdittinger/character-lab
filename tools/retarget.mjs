...
function category(n){if(/Death|Hit|Knock/.test(n))return'Hit and death';if(/Idle|Holding_[ABC]|_Pose$|Aiming|Blocking|Crouching/.test(n))return'Idle';if(/Run|Jog|Sprint/.test(n))return'Run';if(/Jump|Fall|Climb|Roll|Dodge|Slide|Dash/.test(n))return'Jump and movement';if(/Walk|Crawl|Sneak|Swim/.test(n))return'Walk';if(/Death|Hit|Knock/.test(n))return'Hit and death';if(/Melee|Sword|Punch|Ranged|Pistol|Spell|Shield|Throw/.test(n))return'Combat';if(/Farm|Chop|Dig|Fish|Hammer|Pickax|Saw|Work|Lockpick/.test(n))return'Work';return'Gestures and other';}
